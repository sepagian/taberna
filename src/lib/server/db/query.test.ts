...
import { createTestDb, seedTestData, seedTestUser, TEST_USER_ID } from "$test/db";
...
describe("ListQueries", () => {
  ...
  describe("update", () => {
    it("updates list name when owned by user", async () => {
      ...
      const updated = await ListQueries.update(db, "list-1", TEST_USER_ID, "Belanja Baru");
    ...
    it("returns undefined when not owned by user", async () => {
      ...
      const updated = await ListQueries.update(db, "list-1", "other-user", "Stolen");
    ...
    it("returns undefined for non-existent list", async () => {
      ...
      const updated = await ListQueries.update(db, "nonexistent", TEST_USER_ID, "Test");
